...
\\* For the dataset $\left\{\left(x_{i}, y_{i}\right)\right\}_{i=1}^{5} = \left\{\left(-1, \text{\;red\;}\right), \left(-g, \text{\;green\;}\right), \left(-r, \text{\;red\;}\right), \left(g, \text{\;green\;}\right), \left(1, \text{\;blue\;}\right)\right\}$, with some $r , g \in \left(0, 1\right)$ and $r  < g < 2 r $, the minimum loss is $\dfrac{2}{3} \left(g - r\right)^{2}$ and it occurs when $t  = \dfrac{2}{3} g + \dfrac{1}{3} r > r $ when every agent reports truthfully. In this case, agent (-$2, \text{\;red\;}$) is misclassified as green. However, if (-$2, \text{\;red\;}$) misreports as blue, the minimum loss is $\dfrac{2}{3} \left(g + r\right)^{2}$ and it occurs when $t  = \dfrac{2}{3} g - \dfrac{1}{3} r < r $. In this case, agent (-$2, \text{\;red\;}$) is classified correctly, and in particular, she improves the loss from $g  - r $ to $0$.
\newline \newline
...
A strictly convex loss margin classifier, $h^\star $, for three classes with thresholds symmetric around $0$, is an empirical risk learning algorithm, $\left(\mathcal{H}, \ell, S \right)$, with hypothesis class,
\begin{align*}
\mathcal{H} &= \left\{ h_{t} : \mathbb{R} \to  \left\{\text{\;red\;}, \text{\;green\;}, \text{\;blue\;}\right\} \right\}_{t \geq  0},
\end{align*}
where,
...
\text{\;blue\;}& \text{\;if\;} x  > t, \\
\end{array}\right. \]
and loss function $\ell$,
\begin{align*}
\ell\left(x_{i}, y_{i}; h\right) &= f\left(d\left(x_{i}, \left\{x : h\left(x\right) = y_{i}\right\}\right), h \in \mathcal{H},\right.
\end{align*}
where $d\left(x, S \right) $ is the shortest distance from the point $x  \in \mathbb{R}$ to a set $S  \subseteq \mathbb{R}$,
\\* and $f $ is strictly convex, minimized at $0$, with $f\left(0\right)  = 0$,
\\* and training set $S  = \left\{\left(x_{i}, y_{i}\right)\right\}_{i=1}^{n}, x_{i} \in \mathbb{R}, y_{i} \in \left\{\text{\;red\;}, \text{\;green\;}, \text{\;blue\;}\right\}$,
\begin{align*}
h^\star  &= \arg\displaystyle\min_{h \in \mathcal{H}} \displaystyle\sum_{i=1}^{n} \ell\left(x_{i}, y_{i}; h\right).
\end{align*}\end{df}
The example described previously is a squared loss margin classifier with convex function $f\left(\cdot \right)  = \cdot ^{2.}$
\newline \newline
\begin{lem} \label{lem:ex1} 
Define the dataset,
\begin{align*}
S_{1} &= \left\{\left(x_{i}, y_{i}\right)\right\}_{i=1}^{5} = \left\{\left(-1, \text{\;red\;}\right), \left(-g, \text{\;green\;}\right), \left(-r, \text{\;red\;}\right), \left(g, \text{\;green\;}\right), \left(1, \text{\;blue\;}\right)\right\},
\end{align*}
with r, $g \in \left(0, 1\right). S_{1}$ is incentive incompatible for the squared margin classifier with thresholds symmetric around $0$ if $r  < g < 2 r. $
\end{lem}
